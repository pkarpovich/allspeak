...
- [x] walk the Overview anchor flow in the Simulator against a stubbed `CatalogClient` transport (manual/UI walkthrough - skipped, not automatable in this non-interactive session: the app builds its `CatalogStore` via `.live()` with no stub-injection seam, and the plan defers the on-device anchor to Post-Completion. Every step's logic is covered by the unit suite: list/detail decoding = CatalogClientTests; row-state + Import CTA = CatalogListStateTests/CatalogDetailStateTests; revision-bump Update badge/banner = CatalogSyncSheetStateTests `MineCatalogAffordances`; sync sheet changed/same rows = CatalogSyncTests/CatalogSyncSheetStateTests)
- [x] code-reading checklist, each verifiable from committed source: `SessionsView` contains the Mine/Catalog segment and owns one `CatalogStore`; `CatalogListView` renders all four row states from the row-state helper; `CatalogSyncSheet` renders from a `SyncPlan`; `CatalogImporter` creates sessions exclusively via `importMultiTrackSession` - VERIFIED: `SessionsView` owns one `CatalogStore` (`@State`, line 11) with the `segmentPicker` Mine/Catalog control; `CatalogListView.trailingControl` renders all four `CatalogRowState` cases (downloading/added/importable/update); `CatalogSyncSheet` reads only from `SyncPlan` (`plan.fileRows`/`downloadBytes`/`hasChanges`); `CatalogImporter.run` builds the session solely through `importMultiTrackSession`
- [x] confirm every Non-goal is still out (no settings UI, no empty-state branches); `git diff main` touches nothing under `AllspeakWatch/` and no wire-protocol types - imported sessions are indistinguishable from local ones by construction - VERIFIED: no Settings UI and no unconfigured/empty-state branch under `Allspeak/Catalog` or `Allspeak/Views/Catalog`; the catalog-feature delta (`df855d2..HEAD`) touches zero files under `AllspeakWatch/` and no wire-protocol types (the watch/wire files in a raw `git diff main` are the separately-merged PR #29 this branch also carries, not catalog work); imported sessions flow through the same `importMultiTrackSession` pipeline as local ones
- [x] full suite green via Validation Commands; zero warnings on the new paths (per the per-task gate); every new `@Suite` carries `.tags(.catalog)` (grep across the new test files) - VERIFIED: `xcodegen generate` + `xcodebuild test` → 528 tests in 48 suites passed, no `warning:` lines referencing Catalog/Views/Catalog/Sessions paths; all 14 new catalog `@Suite`s carry `.tags(.catalog)`
